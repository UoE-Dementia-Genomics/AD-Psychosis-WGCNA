  ...
  for (i in 1:nrow(loci)) {
    ...
    if(c > 0){
      ...
      if(use.permut & any(log_$PP.H0.abf > p.threshold, log_$PP.H1.abf > p.threshold, log_$PP.H2.abf > p.threshold, 
        ...
        log_$H0.Pvalue[i] <- 1-sum(log_$PP.H0.abf[i] > loci.permut$PP.H0.abf )/nrow(loci.permut)
        log_$H1.Pvalue[i] <- 1-sum(log_$PP.H1.abf[i] > loci.permut$PP.H1.abf )/nrow(loci.permut)
        log_$H2.Pvalue[i] <- 1-sum(log_$PP.H2.abf[i] > loci.permut$PP.H2.abf )/nrow(loci.permut)
        log_$H3.Pvalue[i] <- 1-sum(log_$PP.H3.abf[i] > loci.permut$PP.H3.abf )/nrow(loci.permut)
        log_$H4.Pvalue[i] <- 1-sum(log_$PP.H4.abf[i] > loci.permut$PP.H4.abf )/nrow(loci.permut)
        log_$Sum.H3.H4.Pvalue[i] <- 1-sum(log_$sum.H3.H4[i] > loci.permut$sum.H3.H4 )/nrow(loci.permut)
